// The alarm, via WebAudio — no audio asset needed. Browsers only allow audio
// after a user gesture, so unlockAudio() must be called from an event handler
// before anything here can be heard.
//
...
// that: hidden pages are clamped to about one callback a minute. So the dings
// are scheduled ahead **on the audio clock**, which runs on the audio thread
// and is not throttled — the interval below only tops the schedule up, and can
// be starved for a full minute without the alarm missing a beat.
