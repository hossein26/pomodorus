...
// throttle counts events in a sliding window, in memory.
//
// In memory rather than in Postgres, and the distinction matters. The per
// address and per IP limits on issuing codes are database counts because they
// have to survive a restart: a code already mailed is a fact about the world,
// and forgetting it would let a restart mint fresh quota. These are different.
// They exist to make bulk abuse expensive inside a window measured in minutes,
// and losing them on a deploy costs one window of protection rather than any
// correctness. Keeping them out of the database is also the point under load:
// a flood is trying to exhaust exactly the resource a database-backed limiter
// would spend on it.
//
...
func newThrottle(window time.Duration, limit int) *throttle {
	return &throttle{window: window, limit: limit, seen: make(map[string][]time.Time)}
}
...
// not record anything.
func (t *throttle) remaining(key string, now time.Time) int {
	t.mu.Lock()
	defer t.mu.Unlock()
...
func (t *throttle) sweepLocked(cutoff time.Time) {
